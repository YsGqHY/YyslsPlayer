...
这也是作者第一个使用 Go 语言编写的项目，主要用于入门 Go 语言、学习桌面应用开发流程，以及实践 Wails、后端服务分层、前后端协作等工程能力。
...
注意：未生成 Wails TypeScript bindings 时，`typecheck` 仍会报 `@bindings/YyslsPlayer/...` 找不到。这不是 MUI 依赖问题，而是本地生成文件缺失。
...
`@mui/material` 已在依赖中声明；如果仍然出现 `TS2307: Cannot find module '@mui/material'`，通常是前端依赖未安装或编辑器没有使用 `frontend/tsconfig.json`。
...
| `pnpm --dir frontend build` | 前端单独构建，生成本地构建产物。 |
| `go mod tidy` | 整理 Go 依赖。 |
| `go test ./...` | 运行 Go 测试。 |
| `wails3 generate bindings` | 生成 Wails TypeScript bindings。 |
...
`main.go` 会嵌入前端构建产物，所以单独执行 `go build .` 前必须先执行 `pnpm --dir frontend build`。

## TS2307 排障

### `Cannot find module '@mui/material'`

原因通常是前端依赖尚未安装，或编辑器打开了错误的 TypeScript 项目。
...
原因是 Wails TypeScript bindings 是本地生成产物，不会随源码仓库提交。
...
不要依赖全局 TypeScript。项目会使用安装后的本地 TypeScript 版本。
...
| macOS | `~/Library/Application Support/YyslsPlayer/yyslsplayer.json` |
| Linux | `$XDG_DATA_HOME/YyslsPlayer/yyslsplayer.json` 或 `~/.local/share/YyslsPlayer/yyslsplayer.json` |

应用内「设置 -> 数据存储」可修改数据文件位置。
...
| 持久化 | 统一走后端 JSON 数据文件，前端禁止用 `localStorage` / `sessionStorage` 做业务持久化。 |
...
GNU General Public License v3.0 only（GPL-3.0-only）。允许使用、复制、修改和分发；分发修改版或二进制时必须按 GPLv3 继续授权并提供对应源码。详见 `LICENSE`。
